#!/usr/bin/env python3
"""
File watcher service that automatically processes new Copilot session files
"""
...
class CopilotSessionHandler(FileSystemEventHandler):
    """Handles new Copilot session files"""

    def __init__(self, config: Config, sessions_dir: Path | None = None, output_dir: Path | None = None):
        self.config = config
...
def main(config: Config | None = None) -> None:
    """Start the filesystem watcher that processes session exports."""
    ...
    logger.info("=" * 60)
    logger.info("🚀 Copilot Session Auto-Organizer")
    logger.info("=" * 60)
    logger.info("📂 Watching directory: %s", sessions_dir)
    logger.info("📦 Output directory: %s", output_dir / 'prompts')
    logger.info("=" * 60)
    event_handler = CopilotSessionHandler(cfg, sessions_dir, output_dir)
    observer = Observer()
